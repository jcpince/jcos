...
static inline void *
read_pointer (const void *p) { const union unaligned *up = (const union unaligned *)p; return up->p; }
...
{
  struct dwarf_eh_bases bases;
  const struct dwarf_fde *fde = _Unwind_Find_FDE ((void*)((addr_t)pc-1), &bases);
  if (fde)
    return bases.func;
  else
    return NULL;
		  ...
		  _Unwind_FrameState *fs)
{
  ...
  const unsigned char *ret = (const unsigned char*)NULL;
  _uleb128_t utmp;
  _sleb128_t stmp;
...
execute_cfa_program (const unsigned char *insn_ptr,
		     ...
		     _Unwind_FrameState *fs)
{
  struct frame_state_reg_info *unused_rs = (struct frame_state_reg_info *)NULL;

  /* Don't allow remember/restore between CIE and FDE programs.  */
  fs->regs.prev = (struct frame_state_reg_info*)NULL;
  ...
  while (insn_ptr < insn_end
	 && fs->pc < (void*)((addr_t)context->ra + _Unwind_IsSignalFrame (context)))
    {
	...
	fs->pc = (void*)((addr_t)fs->pc + (addr_t)(insn & 0x3f) * ((addr_t)fs->code_align));
	...
	case DW_CFA_advance_loc1:
	  fs->pc = (void*)((addr_t)fs->pc + read_1u (insn_ptr) * fs->code_align);
	  insn_ptr += 1;
	  break;
	case DW_CFA_advance_loc2:
	  fs->pc = (void*)((addr_t)fs->pc + read_2u (insn_ptr) * fs->code_align);
	  insn_ptr += 2;
	  break;
	case DW_CFA_advance_loc4:
	  fs->pc = (void*)((addr_t)fs->pc + read_4u (insn_ptr) * fs->code_align);
	  insn_ptr += 4;
	  break;
	...
	case DW_CFA_remember_state:
	  {
		...
		unused_rs = unused_rs->prev;
	      ...
	      new_rs = (struct frame_state_reg_info*)alloca (sizeof (struct frame_state_reg_info));
...
uw_frame_state_for (struct _Unwind_Context *context, _Unwind_FrameState *fs)
{
  ...
  fde = _Unwind_Find_FDE ((void*)((addr_t)context->ra + (addr_t)_Unwind_IsSignalFrame (context) - 1),
			  &context->bases);
  if (fde == NULL)
    {
...
__frame_state_for (void *pc_target, struct frame_state *state_in)
{
  ...
  context.ra = (void*)((addr_t)pc_target + 1);
...
uw_update_context_1 (struct _Unwind_Context *context, _Unwind_FrameState *fs)
{
  ...
  switch (fs->regs.cfa_how)
    ...
    case CFA_REG_OFFSET:
      ...
      cfa = (void*)((addr_t)cfa + fs->regs.cfa_offset);
      break;
  ...
  for (i = 0; i < DWARF_FRAME_REGISTERS + 1; ++i)
	...
	_Unwind_SetGRPtr (context, i,
			  (void *) ((addr_t)cfa + fs->regs.reg[i].loc.offset));
	break;
	...
	_Unwind_SetGRValue (context, i,
			    ...
			    ((addr_t)cfa + fs->regs.reg[i].loc.offset));
	break;
		      ...
		      struct _Unwind_Context *target)
{
  ...
  if (!_Unwind_GetGRPtr (current, __builtin_dwarf_sp_column ()))
	...
	return (addr_t)target_cfa - (addr_t)current->cfa + target->args_size;
      else
	return (addr_t)current->cfa - (addr_t)target_cfa - target->args_size;
    }
  return 0;
}
