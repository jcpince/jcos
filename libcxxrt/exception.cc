...
inline int cust__sync_sub_and_fetch(uintptr_t *count, int subvalue)
{
	*count -= subvalue;
	return *count;
}
...
static void releaseException(__cxa_exception *exception)
{
	...
	if (cust__sync_sub_and_fetch(&exception->referenceCount, 1) == 0)
	{
...
static void report_failure(_Unwind_Reason_Code err, __cxa_exception *thrown_exception)
{
	switch (err)
	{
		...
		case _URC_FATAL_PHASE1_ERROR:
			printk("Fatal error during phase 1 unwinding\n");
			break;
...
extern "C" void __cxa_rethrow()
{
	...
	if (0 == ex)
	{
		printk("Attempting to rethrow an exception that doesn't exist!\n");
		std::terminate();
	}
